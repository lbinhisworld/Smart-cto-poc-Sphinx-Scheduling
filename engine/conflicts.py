...
def earliest_finish_finished(inp: ScheduleInput, finished_wo: Wo) -> date:
    ...
    if route and route.needs_semi and route.semi_item_code:
        ...
        start = next_workday(inp.calendar, finished_wo.group_code, start)
    else:
        start = max(inp.today, finished_wo.earliest_start)
...
def detect_conflicts(inp: ScheduleInput, result: ScheduleResult) -> list[Conflict]:
    ...
    hours_by_group_day: dict[tuple[str, date], Decimal] = defaultdict(Decimal)
    for task in result.tasks:
        key = (task.group_code.value, task.task_date)
        hours_by_group_day[key] += task.hours_wall

    for (group_val, task_date), total_wall in hours_by_group_day.items():
        from engine.models import GroupCode

        group_code = GroupCode(group_val)
        limit = calendar_hours(inp.calendar, group_code, task_date)
        if total_wall > limit:
            conflicts.append(
                Conflict(
                    code="E4",
                    level=ConflictLv.YELLOW,
                    task_id=None,
                    message=f"{group_val} {task_date} 墙钟工时超限",
                    suggest="ADD_CREW",
                )
            )
