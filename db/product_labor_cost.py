...
def planned_labor_cost_by_product(
    ...
    *,
    ...
    if ver <= 0:
        return {"plan_version": ver, "products": [], "totals": {"hours_man": 0.0, "cost_planned": 0.0}}
    ...
    orders = {
        o.order_no: o.item_code
        for o in session.scalars(select(SoOrderRow)).all()
    }
    ...
    for t in tasks:
        ...
        if wo is None:
            continue
        product = orders.get(wo.source_order_no, wo.item_code)
        ...
        by_product[product]["cost_planned"] += labor_cost(h, rate)
        by_product[product]["order_nos"].add(wo.source_order_no)
...
def product_labor_for_item(
    ...
    *,
    ...
    out = {**base, "plan_version": data["plan_version"]}
    if qty_order and qty_order > 0 and row and row["order_count"] > 0:
        # POC：按当前计划均摊到「单」展示（无全量 CTP 时用订单数近似）
