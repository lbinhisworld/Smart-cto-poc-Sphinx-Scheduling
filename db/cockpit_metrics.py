...
def cockpit_snapshot(session: Session, *, today_iso: str = "2026-09-15") -> dict:
    def _biz():
        return ~capacity_fixture_clause()

    order_total = session.scalar(select(func.count()).select_from(SoOrderRow).where(_biz())) or 0
    pending = session.scalar(
        select(func.count())
        .select_from(SoOrderRow)
        .where(_biz(), SoOrderRow.schedule_phase == "PENDING")
    ) or 0
    in_sched = session.scalar(
        select(func.count())
        .select_from(SoOrderRow)
        .where(_biz(), SoOrderRow.schedule_phase == "IN_SCHEDULING")
    ) or 0
    near_due = session.scalar(
        select(func.count())
        .select_from(SoOrderRow)
        .where(_biz(), SoOrderRow.due_date <= __import__("datetime").date.fromisoformat("2026-09-22"))
    ) or 0
    ...
    load = pending_load(session, today=date.fromisoformat(today_iso))
    ...
    return {
        ...
        "schedule_progress": {
            ...
            "pending_component_count": len(load["components"]),
            "pending_wall": load["hours"]["wall"],
            "pending_man": load["hours"]["man"],
            "pending_label": load["label"],
            "uncomputable_count": load["uncomputable_count"],
        },
        "labor_cost": {
            "plan_version": ver,
            "totals": labor_totals,
