...
from engine.models import CalendarDay, Dept, GroupCode, Sph, UomConvert
...
def next_workday(
    ...
    cursor = start
    for _ in range(366):
        if is_workday(calendar, dept, group_code, cursor):
            return cursor
...
def count_workdays_forward(
    ...
    remaining = boards
    cursor = next_workday(calendar, dept, group_code, start)
    ...
    while remaining > 0:
        ...
        if guard > 365:
            break
        if not is_workday(calendar, dept, group_code, cursor):
            cursor += timedelta(days=1)
            continue
        cap = day_capacity(
            calendar,
