...
class TraceEvent(_Model):
    ...
    qty_board: int | None = None
    remaining_after: int | None = None
    cap_board: int | None = None
    occupied_before: int | None = None
...
class BomLine(_Model):
    ...
    qty_basis_uom: Uom = Uom.BOX
    scrap_rate: Decimal | None = None
    offset_days: int = 0
    lead_time_days: int = 4
    kit_critical: bool = True


class KitMode(str, Enum):
    WARN = "WARN"
    STRICT = "STRICT"


class KitAllocation(_Model):
    order_no: str
    component_item_code: str
    qty_board: int
...
class KitLineResult(_Model):
    ...
    from_stock: int
    net_wo_board: int
    shortage_board: int = 0
    ready_date: date | None = None


class KitCheckResult(_Model):
    order_no: str
    finished_wo_no: str
    kit_ready_date: date | None = None
    is_kitted: bool = True
    finished_plan_start: date | None = None
    lines: list[KitLineResult] = Field(default_factory=list)
...
class Conflict(_Model):
    ...
    hours_wall_total: Decimal | None = None
    hours_wall_limit: Decimal | None = None
...
class ScheduleInput(_Model):
    ...
    bom_lines: dict[str, list[BomLine]] = Field(default_factory=dict)
    ...
    def bom_for(self, parent_item_code: str) -> list[BomLine]:
        if parent_item_code in self.bom_lines and self.bom_lines[parent_item_code]:
            return sorted(self.bom_lines[parent_item_code], key=lambda ln: ln.line_no)
        route = self.routes.get(parent_item_code)
        if route and route.needs_semi and route.semi_item_code and route.semi_board_per_box:
            return [
                BomLine(
                    parent_item_code=parent_item_code,
                    line_no=1,
                    component_item_code=route.semi_item_code,
                    component_role=ComponentRole.SEMI,
                    qty_per_parent=route.semi_board_per_box,
                    qty_basis_uom=Uom.BOX,
